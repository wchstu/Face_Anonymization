...
class FaceDetector(object):
    def __init__(self, out_postfix='_dsfd.pkl', detection_model_path='weights/WIDERFace_DSFD_RES152.pth',
        ...
        self.transform = TestBaseTransform((104, 117, 123))     # 三通道分别减去104, 117, 123
        self.cfg = widerface_640    # 参数信息
        self.thresh = self.cfg['conf_thresh']   # 0.01
    ...
    def __call__(self, input_path, output_path=None):
        ...
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))       # 总的帧数
        fps = cap.get(cv2.CAP_PROP_FPS)                             # 帧率
        input_vid_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))    # 图像宽度
        input_vid_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))  # 图像高度
        image_size = (input_vid_height, input_vid_width)

        # Set default tensor type
        torch.set_default_tensor_type('torch.cuda.FloatTensor')     # 设置默认数据格式

        # For each frame in the video
        frame_bgr_list = []     # bgr
        frame_tensor_list = []  # Tensor
        det_list = []           # determinant
        for i in tqdm(range(total_frames), unit='frames'):
            ...
            frame_tensor = torch.from_numpy(self.transform(frame)[0]).permute(2, 0, 1).unsqueeze(0).to(self.device) # 图像减去各通道均值后转为Tensor，[1, 3, height, width]
            frame_tensor_list.append(frame_tensor)
            if len(frame_tensor_list) < self.batch_size and (i + 1) < total_frames:
                continue
            frame_tensor_batch = torch.cat(frame_tensor_list, dim=0)    # 将一个batch的tensor合并 [batch_size, 3, height, width]

            # Process
            detections_batch = self.net(frame_tensor_batch)     # 检测一个batch的图像 [batch_size, 2, 5000, 5]，置信度由大到小排列
            ...
            for b, detections in enumerate(detections_batch):
                ...
                for i in range(detections.size(1)):
                    j = 0
                    while detections[0, i, j, 0] >= self.thresh:    # 筛选出可能的候选框
                        curr_det = detections[0, i, j, [1, 2, 3, 4, 0]].cpu().numpy()
                        curr_det[:4] *= scale.cpu().numpy()         # 计算box在图像中的坐标
                        det.append(curr_det)
                        j += 1

                if len(det) == 0:
                    det_list.append(np.array([], dtype='float32'))  # 未检测到人脸保存为空array
                else:
                    det = np.row_stack((det))   # 将候选框及置信度转为np.array，并按行排列
                    # if det.shape[0] > 1:
                    #     det = bbox_vote(det.astype(float))
                    det_filtered = det[det[:, 4] > 0.5, :4]     # 选取置信度大于0.5的边界框并保存
                    det_list.append(det_filtered)

                # Render
                if self.verbose > 0:
                    det_display = np.round(det_filtered).astype(int)    # 边界框取整
                    render_img = frame_bgr_list[b]                      #
                    for rect in det_display:
                        # cv2.rectangle(render_img, tuple(rect[:2]), tuple(rect[:2] + rect[2:]), (0, 0, 255), 1)
                        cv2.rectangle(render_img, tuple(rect[:2]), tuple(rect[2:]), (0, 0, 255), 1) # 绘制人脸边界框
            ...
            # Clear lists
            frame_bgr_list.clear()      # 释放内存
            frame_tensor_list.clear()   # 释放内存

        # Write to file
        with open(output_path, 'wb') as f:
            pickle.dump(det_list, f)    # 保存边界框

        # Reset default tensor type
        torch.set_default_tensor_type('torch.FloatTensor')  # 更改torch数据类型为预设值
