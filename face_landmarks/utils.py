...
def filter_landmarks(landmarks, threshold=0.5):
    ...
    """
    landmarks_min = landmarks.view(landmarks.shape[:2] + (-1,)).min(2)[0].view(landmarks.shape[:2] + (1, 1))    # 计算每一通道的最小值
    landmarks_max = landmarks.view(landmarks.shape[:2] + (-1,)).max(2)[0].view(landmarks.shape[:2] + (1, 1))    # 计算每一通道的最大值
    landmarks = (landmarks - landmarks_min) / (landmarks_max - landmarks_min)   # landmarks化为[0, 1]
    # landmarks.pow_(2)
    landmarks[landmarks < threshold] = 0.0  # 将landmarks小于阈值的点赋值为0
...
class LandmarksHeatMapEncoder(nn.Module):
    ...
    def __call__(self, landmarks):
        ...
        w = landmarks.div(landmarks.view(landmarks.shape[:2] + (-1,)).sum(dim=2).view(landmarks.shape[:2] + (1, 1)))    # 每一通道除以该通道元素和
        x = w * self.x_indices
        y = w * self.y_indices
...
if __name__ == '__main__':
    model = LandmarksHeatMapDecoder(size=256)
    x = torch.Tensor([128, 128])
    x = x.reshape(1, 1, 2)
    y = model(x)
    print(torch.max(y), torch.min(y))
    encoder = LandmarksHeatMapEncoder(size=256)
    z = encoder(y)
    print(z)
