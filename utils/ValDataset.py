...
if __name__ == '__main__':
    ...
    dataset = FNDataset('/home/wang/Dataset/CelebA', transform=transform)
    loader = torch.utils.data.DataLoader(dataset=dataset, batch_size=64, num_workers=10, shuffle=True, drop_last=True)
    from tqdm import tqdm
    for data in tqdm(loader):
        img, iden_img, msk, lndm, label, name = data
        print(lndm)
        # img_background = img * msk
        # torchvision.utils.save_image(img_background.add(1).mul(0.5), 'im.jpg')
        exit()